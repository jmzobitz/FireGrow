...
### Purpose: Read in the Ibutton data from Kajar



# Load in each dataset:
N1990 <- readxl::read_xlsx('data-raw/kajar/Fire N1990 daily averages T.xlsx',sheet="Concluding") |> select(1:4) |>
  rename(Date=1,L5=2,L10=3,L30=4) |>
  mutate(source="iButton",ID="N1990",Date=as.Date(Date))

N2012 <- readxl::read_xlsx('data-raw/kajar/Fire N2012 daily average T.xlsx',sheet="Concluding") |> select(1:4) |>
  rename(Date=1,L5=2,L10=3,L30=4) |>
  mutate(source="iButton",ID="N2012",Date=as.Date(Date))

NC <- readxl::read_xlsx('data-raw/kajar/NControl daily average T.xlsx',sheet="Concluding") |> select(1:4) |>
  rename(Date=1,L5=2,L10=3,L30=4) |>
...
# Save the data
usethis::use_data(soil_temperature_iButton_data,overwrite=TRUE)
