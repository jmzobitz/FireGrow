...
### Purpose: Estimate the diffusivity from our measurements to compute D.
...
# Estimate the diffusivity from an average between values
small_data <- diffusivity |> filter(ID !='N2012') |> group_by(Date,layer) |> summarize(across(rate:diffusivity,~mean(.x))) |> mutate(ID='N1968',real=FALSE)


# Then join things back together:

diffusivity <- rbind(mutate(diffusivity,real=TRUE),small_data)
