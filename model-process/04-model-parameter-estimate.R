...
run_model_revised_random <- function(input_model_data,
  ...
  output_check <- output_n |>
    summarize(rss=sum((Rsoil-medianR)^2),
              error = bulk_rS_err,
              log_likely = -sum(stats::dnorm(medianR, mean = Rsoil, sd = error, log = TRUE)),
              accept = FALSE,
...
### Test function to show how we can move through things
# One MCMC chain
metropolis_chain_test <- function(chain_id, n_sims, model_data, field_data, start_params, tuning = FALSE) {
...
metropolis_estimate_mcmc_test <- function(model_data, field_data, parameters, n_sims=20000, n_chains = 5,chain_sims = 10000) {
  ...
  if (is_windows) {
    ...
    results <- parLapply(cl, 1:n_chains, function(chain_id) {
      metropolis_chain_test(chain_id, chain_sims, model_data, field_data, parameters, tuning = TRUE)
    })
  ...
  final_results_pre <- metropolis_chain_test(chain_id = 0,
                                    n_sims = n_sims,
                                    model_data = model_data,
                                    field_data = field_data,
                                    start_params = new_params,
  ...
  final_results <- rbind(bind_rows(results),final_results_pre) |>
    select(-accept,-finite_rss,-params,-fluxes,-error) |>
    mutate(sims = if_else(chain == 0, sims+n_sims,sims))
...
for (i in 1:nrow(forcing_model_ready_input2)) {
  ...
  out_tibble <- metropolis_estimate_mcmc_test(model_data = forcing_model_ready_input2$modeling_data[[i]],
                                         field_data = forcing_model_ready_input2$field_data[[i]],
                                         parameters = forcing_model_ready_input2$parameters[[i]],
                                         n_sims = curr_sims,chain_sims = tot_chain_sims,n_chains = tot_chains)

  filename <- paste0('parameter-estimation-outputs/estimates-test/',
                     forcing_model_ready_input2$Year[[i]],'--',
                     forcing_model_ready_input2$model[[i]],'--',
                     forcing_model_ready_input2$depth[[i]],'.Rda')
...
### Can we do a smaller dataset?
out_estimates <- metropolis_estimate_mcmc_test(model_data = forcing_model_ready_input2$modeling_data[[i]],
                                          field_data = forcing_model_ready_input2$field_data[[i]],
                                          parameters = forcing_model_ready_input2$parameters[[i]],
                                          n_sims = curr_sims,
                                          chain_sims = tot_chain_sims, n_chains = tot_chains)
