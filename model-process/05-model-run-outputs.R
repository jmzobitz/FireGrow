...
model_sensitivity_data <- tibble(
  Year = map_chr(.x=input_files,.f=~str_extract(.x,pattern="(?<=estimates/)[:alnum:]+(?=--)")),
...
save(model_sensitivity_data,file='parameter-estimation-outputs/parameter-estimate-summary.Rda')
