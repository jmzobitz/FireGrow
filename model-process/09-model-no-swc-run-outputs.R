...
input_files <- list.files(path = 'parameter-estimation-outputs/estimates-linear-swc',pattern="^N",full.names = TRUE)
...
model_sensitivity_data <- tibble(
  Year = map_chr(.x=input_files,.f=~str_extract(.x,pattern="(?<=estimates-linear-swc/)[:alnum:]+(?=--)")),
  model = map_chr(.x=input_files,.f=~str_extract(.x,pattern="(?<=--)[:graph:]+(?=--)")),
  depth = map_chr(.x=input_files,.f=~str_extract(.x,pattern="T_soil_[:digit:]{1,2}")),
  # value = histogram_data,
  params = out_params,
...
save(model_sensitivity_data,file='parameter-estimation-outputs/parameter-estimate-linear-swc-summary.Rda')
